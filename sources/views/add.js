	...
	config(){
		...
		const buttons = {
			...
			cols:[
				...
				{
					view:"button", value:_("Add"), type:"form", autowidth:true,
					tooltip:"Add player",
					click:() => {
						if (this.getRoot().validate()){
							...
							data[num].id = num;
							data[num].fname = this.$$("firstname").value();
							data[num].lname = this.$$("lastname").value();
							data[num].position = this.$$("position:combo").value();
							data[num].city = this.$$("cities:combo").value();
							data[num].email = this.$$("mail").value();
							data[num].club = this.$$("cclub").value();
							data[num].birthday = this.$$("birthdate").value();
							data[num].notes = this.$$("notes").value();
